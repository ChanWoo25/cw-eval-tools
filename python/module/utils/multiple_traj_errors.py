...
EST_TRAJ_FILENAME = 'est_traj.txt'
EST_TRAJ_FORMAT = '%f %f %f %f %f %f %f %f' # (cols:8) Time, px, py, pz, qx, qy, qz, qw

class SingleTrajectoryError():
    def __init__(self, result_dir:str, use_cache:bool):
        self.result_dir:str = result_dir
        self.parent_dir:str = os.path.dirname(result_dir)
        self.gt_filename:str = os.path.join(self.parent_dir, GT_FILENAME)
        self.est_traj_filename:str = os.path.join(self.parent_dir, EST_TRAJ_FILENAME)
        self.use_cache:bool = use_cache

        self.abs_t_error = np.ndarray(0)
        self.abs_R_error = np.ndarray(0)
        self.abs_s_error = np.ndarray(0)
        self.rel_t_error = np.ndarray(0)
        self.rel_R_error = np.ndarray(0)
        self.rel_R_per_m_error = np.ndarray(0)

        self.rmse_abs_t_error:float = None
        self.rmse_abs_R_error:float = None
        self.rmse_abs_s_error:float = None
        self.rmse_rel_t_error:float = None
        self.rmse_rel_R_error:float = None
        self.rmse_rel_R_per_m_error:float = None

        self.trajectory_length:int = None

        self.id_estimate_type:str = None
        self.id_algorithm:str = None
        self.id_dataset:str = None

    def summary(self) -> str:
        """Return a one-line summary for a single trajectory"""
        summary:str = "[SingelTrajError], "
        summary += "length(%d), "%self.trajectory_length
        summary += "type(%s), "%self.id_estimate_type
        summary += "algo(%s), "%self.id_algorithm
        summary += "dataset(%s), "%self.id_dataset
        summary += "ate(%.4f), "%self.rmse_abs_t_error
        summary += "aRe(%.4f), "%self.rmse_abs_R_error
        summary += "ase(%.4f), "%self.rmse_abs_s_error
        summary += "rte(%.4f), "%self.rmse_rel_t_error
        summary += "rRe(%.4f), "%self.rmse_rel_R_error
        summary += "rRpMe(%.4f)"%self.rmse_rel_R_per_m_error
        return summary
