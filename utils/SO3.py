...
class SO3:
    ...
    eps_ = 1e-8
    I3 = torch.eye(3).float().to(device_)

    @classmethod
    def exp(cls, phi):
        angle = phi.norm(dim=1, keepdim=True)
        mask = angle[:, 0] < cls.eps_
        dim_batch = phi.shape[0]
        I3 = cls.I3.expand(dim_batch, 3, 3)
        ...
        Rot[mask] = I3[mask] + SO3.wedge(phi[mask])
        Rot[~mask] = c*I3[~mask] + \
    ...
    def to_quaternion(cls, Rots, ordering='wxyz'):
        ...
        near_zero_mask = qw.abs() < cls.eps_
    ...
    def normalize(cls, Rots):
        ...
        S = cls.I3.clone().repeat(Rots.shape[0], 1, 1)
    ...
    def qmul(cls, q, r, ordering='wxyz'):
        ...
        if ordering == 'xyzw':
            ...
            print('[qmul] -- Ordring error'); exit(2)

        terms = cls.bouter(r, q_tmp)
