...
if __name__ == '__main__':
    ...
    source_fn = source_dir / 'estimate.txt'
    output_fn = source_dir / 'result.txt'

    data = torch.from_numpy(np.loadtxt(source_fn)).cuda()
    ...
    dy = ty[1:] - ty[:-1]
    dz = tz[1:] - tz[:-1]
    total_length = torch.sqrt(dx*dx + dy*dy + dz*dz).sum().item()
    print('total_length: %.4f' % total_length)

    e2e_dx = tx[-1] - tx[0]
    e2e_dy = ty[-1] - ty[0]
    e2e_dz = tz[-1] - tz[0]
    end2end_error = torch.sqrt(e2e_dx*e2e_dx +
                               e2e_dy*e2e_dy +
                               e2e_dz*e2e_dz).item()
    print('end2end_error: %.4f' % end2end_error)

    percentage = 100.0 - 100.0 * (end2end_error / total_length)
